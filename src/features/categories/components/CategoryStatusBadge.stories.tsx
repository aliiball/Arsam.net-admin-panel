...
import { CATEGORY_STATUSES } from '../data/categories';

const meta = {
  title: 'Categories/CategoryStatusBadge',
  component: CategoryStatusBadge,
  parameters: { layout: 'centered' },
  args: { status: 'active' },
} satisfies Meta<typeof CategoryStatusBadge>;
  ...
  render: () => (
    <div className="flex flex-wrap gap-2">
      {CATEGORY_STATUSES.map((s) => (
        <CategoryStatusBadge key={s} status={s} />
      ))}
    </div>
  ),
  play: async ({ canvas }) => {
